...
'''
Decoupling config from queries to avoid Streamlit crashing from config
...
YELLOW_TAXI_PROCESSED_FOLDER = os.getenv("YELLOW_TAXI_PROCESSED_FOLDER")

# Available years query

AVAILABLE_YEARS_QUERY = f"""
SELECT DISTINCT year
FROM {YELLOW_TAXI_TABLE_NAME}
ORDER BY year DESC
"""

# Base table
YELLOW_TAXI_PROCESSED_CREATE_QUERY = f"""
CREATE EXTERNAL TABLE {YELLOW_TAXI_TABLE_NAME} (
  `vendorid` int, 
...
LOCATION
  's3://{YELLOW_TAXI_BUCKET_NAME}/{YELLOW_TAXI_PROCESSED_FOLDER}'
TBLPROPERTIES (
  ...
  'storage.location.template'='s3://{YELLOW_TAXI_BUCKET_NAME}/{YELLOW_TAXI_PROCESSED_FOLDER}/year=${{year}}/month=${{month}}/'
)
...
CREATE OR REPLACE VIEW taxi_data.{YELLOW_TAXI_ENRICHED_TABLE_NAME} AS
SELECT
    *,
    DATE_PARSE(
...
FROM
  {YELLOW_TAXI_ENRICHED_TABLE_NAME}
...
FROM {YELLOW_TAXI_ENRICHED_TABLE_NAME}
